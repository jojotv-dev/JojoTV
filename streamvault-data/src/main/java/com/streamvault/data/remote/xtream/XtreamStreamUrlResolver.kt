...
class XtreamStreamUrlResolver @Inject constructor(
    ...
    private data class CachedResolvedUrl(
        val resolved: ResolvedStreamUrl,
        val cachedAtMs: Long = System.currentTimeMillis()
    ) {
        fun isValid(): Boolean {
            val now = System.currentTimeMillis()
            val expiry = resolved.expirationTime
            return if (expiry != null) now < expiry * 1000 else now - cachedAtMs < 60_000L
        }
    }
    private val resolvedUrlCache = ConcurrentHashMap<String, CachedResolvedUrl>()
    ...
    suspend fun resolveWithMetadata(
        ...
        resolvedUrlCache[url]?.takeIf { it.isValid() }?.let { return it.resolved }
        ...
        val result = when (resolvedProvider.type) {
        ...
        result?.also { resolvedUrlCache[url] = CachedResolvedUrl(it) }
        return result
    }
